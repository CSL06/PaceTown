...
BASE = ROOT / "assets" / "production" / "character-redesign-v3" / "base-sprites-64x96" / "sky"
POSE_MASTER = ROOT / "assets" / "generated" / "characters" / "sky" / "sky-animation-pose-master-clean.png"
OUT = ROOT / "assets" / "production" / "character-redesign-v3" / "sky-variant" / "sky"
CELL = (64, 96)
...
def demo(lookup: dict[str, Image.Image]):
    ...
    images[0].save(OUT / "sky-animation-showcase.gif", save_all=True, append_images=images[1:], duration=durations, loop=0, disposal=2)
...
def main():
    ...
    sheet.save(OUT / "sky-animation-sheet.png")
    (OUT / "sky-atlas.json").write_text(json.dumps({"frames": atlas, "meta": {"image": "sky-animation-sheet.png", "size": {"w": 512, "h": 480}, "format": "RGBA8888", "scale": "1"}}, indent=2), encoding="utf-8")
    (OUT / "sky-animations.json").write_text(json.dumps(animations, indent=2), encoding="utf-8")
    manifest = {"character": "sky", "package": "sky-variant", "cell": {"width": 64, "height": 96}, "frameCount": 40, "animationCount": 18, "identitySource": "assets/generated/references/sky-user-reference.png", "poseSource": "assets/generated/characters/sky/sky-animation-pose-master-clean.png", "cleanup": ["fixed 24-color palette", "binary alpha", "BOX reduction", "consistent pose scale and baseline"]}
    (OUT / "sky-animation-manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    demo(lookup); print(f"Generated clean Sky variant package in {OUT}")
