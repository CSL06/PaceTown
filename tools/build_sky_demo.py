...
from build_sky_variant import CELL, ROOT, font, shift, split_poses


OUT = ROOT / "assets" / "production" / "character-redesign-v3" / "sky-demo" / "sky"
...
def save_demo(lookup: dict[str, Image.Image]) -> None:
    ...
    images[0].save(OUT / "sky-demo.gif", save_all=True, append_images=images[1:], duration=durations, loop=0, disposal=2)
...
def main() -> None:
    ...
    sheet.save(OUT / "sky-sheet.png")
    (OUT / "sky-atlas.json").write_text(json.dumps({"frames": atlas, "meta": {"image": "sky-sheet.png", "format": "RGBA8888", "size": {"w": 256, "h": 288}, "scale": "1"}}, indent=2), encoding="utf-8")
    (OUT / "sky-animations.json").write_text(json.dumps(animations, indent=2), encoding="utf-8")
    manifest = {
        "character": "sky", "package": "sky-demo",
        ...
        "source": "assets/generated/characters/sky/sky-animation-pose-master-clean.png",
        "savings": "Reuses existing art and omits 28 unnecessary directional movement frames.",
    }
    (OUT / "sky-manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    save_demo(lookup)
